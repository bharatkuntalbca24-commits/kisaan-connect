...
import { ArrowLeft, Calendar, Clock, MapPin, Phone, CheckCircle2, Loader2 } from 'lucide-react';
import { useNavigate, useParams } from 'react-router-dom';
import { Button } from '@/components/ui/button';
import { cn } from '@/lib/utils';
import { useAuth } from '@/context/AuthContext';
import { getEquipmentById, createBooking } from '@/lib/db';
...
export default function BookEquipment() {
  ...
  const totalPrice = equipment ? equipment.pricePerHour * duration : 0;
  ...
  const handleConfirm = async () => {
    if (!user || !equipment || !id) return;
    setBookingLoading(true);
    try {
      await createBooking(user.uid, id, equipment, {
        date: selectedDate,
        time: selectedTime,
        duration,
        paymentMode,
      ...
      toast({ title: 'बुकिंग विफल', description: err.message, variant: 'destructive' });
    } finally {
  ...
  if (isBooked) {
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center p-6 text-center">
        ...
        <h1 className="text-2xl font-bold text-foreground mb-2">बुकिंग सफल! ✅</h1>
        ...
        <div className="flex gap-3 w-full max-w-sm">
          ...
          <Button className="flex-1 h-14 bg-primary" onClick={() => navigate('/bookings')}>
            बुकिंग देखें
          </Button>
        </div>
      </div>
          ...
          <div className="animate-fade-in">
            ...
            <div className="space-y-3 mb-8">
              <button
                ...
                className={cn(
                  ...
                  paymentMode === 'cash' ? 'bg-primary text-primary-foreground' : 'bg-card border border-border'
                )}
              >
                <span className="text-3xl">💵</span>
                <div className="text-left">
                  <h3 className="font-bold">नकद</h3>
                  <p className={cn('text-sm', paymentMode === 'cash' ? 'text-primary-foreground/80' : 'text-muted-foreground')}>
              ...
              <button
                ...
                className={cn(
                  'w-full p-5 rounded-2xl flex items-center gap-4 transition-all',
                  paymentMode === 'upi' ? 'bg-primary text-primary-foreground' : 'bg-card border border-border'
                )}
              >
                <span className="text-3xl">📱</span>
                <div className="text-left">
                  <h3 className="font-bold">UPI</h3>
                  <p className={cn('text-sm', paymentMode === 'upi' ? 'text-primary-foreground/80' : 'text-muted-foreground')}>
            ...
            <div className="bg-card rounded-2xl p-4">
              ...
              <div className="space-y-2 text-sm text-muted-foreground mb-4">
                <p className="flex items-center gap-2"><span className="text-lg">🚜</span>{equipment.nameHindi}</p>
                <p className="flex items-center gap-2"><span className="text-lg">👨‍🌾</span>{equipment.ownerName}</p>
                <p className="flex items-center gap-2">
                  <Calendar className="h-4 w-4" />
                  {new Date(selectedDate).toLocaleDateString('hi-IN', { day: 'numeric', month: 'long' })} - {selectedTime}
                </p>
                <p className="flex items-center gap-2"><Clock className="h-4 w-4" />{duration} घंटे</p>
              </div>
              <div className="flex justify-between items-center pt-3 border-t border-border">
                <span className="font-bold">कुल राशि:</span>
                <span className="text-2xl font-bold text-primary">₹{totalPrice}</span>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-background border-t border-border">
        <div className="flex gap-3">
          ...
          {currentStep < 3 ? (
            ...
            >
              {bookingLoading ? <Loader2 className="h-5 w-5 animate-spin" /> : 'बुकिंग पक्की करें ✓'}
            </Button>
          )}
        </div>
      </div>
